...
public class _Endless2dController : MonoBehaviour
{
    ...
    public bool canMove,isGrounded,airShift;//is my player colliding with the floor? can he shift lanes in air
    ...
    void MoveObj()
    {
        ...
        if (isController == true)
        {
            ...
            if((isGrounded == false && airShift==true )|| isGrounded == true)
            {
    ...
    public void Shoot(GameObject bulz , float force)
    {
        ...
        Ray targetPoint = gameStateManager.endlessCam.ScreenPointToRay(Input.mousePosition);

        Vector3 direction = Vector3.zero;

        Vector3 pos = targetPoint.GetPoint(0);

        camOffsetY = gameStateManager.endlessCam.GetComponent<CameraTracking>().yPos;
       

        direction = pos - new Vector3(gameStateManager.endlessCam.transform.position.x, gameStateManager.endlessCam.transform.position.y - camOffsetY + yDis , gameStateManager.endlessCam.transform.position.z);
    ...
    public void Combo()
    {
        ...
        direction = pos - new Vector3(gameStateManager.endlessCam.transform.position.x, gameStateManager.endlessCam.transform.position.y - camOffsetY, gameStateManager.endlessCam.transform.position.z);
